...
class BaseTask(BaseModel):
    ...
    assignee_id: Optional[int] = Field(None, description="主负责人ID")
    assignee_ids: list[int] = Field(default_factory=list, description="参与人ID列表")
    ...
    def set_status_progress(self):
        if self.assignee_ids:
            self.assignee_id = self.assignee_ids[0]
        elif self.assignee_id is not None:
            self.assignee_ids = [self.assignee_id]
