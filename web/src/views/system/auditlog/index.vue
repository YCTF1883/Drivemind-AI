...
async function handleCleanupAuditLogs() {
  const res = await api.cleanupAuditLogs({ days: cleanupDays.value || 30 })
  const deletedCount = res?.data?.deleted_count ?? 0
  $message.success(`已清理 ${deletedCount} 条历史日志`)
  $table.value?.handleSearch()
}
...
<template>
  <CommonPage>
    <template #action>
      <div class="auditlog-actions">
        <span class="auditlog-actions__label">保留最近</span>
        <NInputNumber
          v-model:value="cleanupDays"
          :min="1"
          :max="3650"
          :show-button="false"
          class="auditlog-actions__input"
        />
        <span class="auditlog-actions__label">天</span>
        <NPopconfirm @positive-click="handleCleanupAuditLogs">
          <template #trigger>
            <NButton type="warning">清理历史日志</NButton>
          </template>
          确认清理 {{ cleanupDays || 30 }} 天前的审计日志？此操作不可恢复。
        </NPopconfirm>
      </div>
    </template>
...
.auditlog-actions__label {
  color: var(--text-color-2);
  white-space: nowrap;
}

.auditlog-actions__input {
  width: 96px;
}
</style>
